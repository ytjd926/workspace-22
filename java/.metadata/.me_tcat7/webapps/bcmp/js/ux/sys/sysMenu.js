	...
	var _this = {
		...
		addLine: function(data){
			...
			$(".remove-btn",line).click(function(){
				ian.confirm('Confirm','Are you sure you want to delete record?',function(r){
					if(r){
						_this.delLine(line);
					}
				})
  				...
  				title:'Menu List',
	   			...
	   			columns:[[
					...
					{field:'childMenus',title:'子目录',width:120,align:'center',formatter:function(value,row,index){
						var html ="<a href='#' onclick='ian.sysMenu.toList("+row.id+")'>子菜单管理("+row.subCount+")</a>";
						return html;
					}}
				]],
				toolbar:[
		...
		init:function(){
			...
			$('#delAllLine_btn').click(function(){
				ian.confirm('确认','确定要删除么？',function(r){
