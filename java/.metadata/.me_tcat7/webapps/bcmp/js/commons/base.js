...
var ian={
	...
	checkLogin:function(data){//检查是否登录超时
		if(data.logoutFlag){
			ian.closeProgress();
			ian.alert('提示',"登录超时,点击确定重新登录.",'error',ian.toLogin);
			return false;
		}
		return true;
	},
			 	...
			 	error:function(response, textStatus, errorThrown){
			 		try{
				 		...
				 		if(!ian.checkLogin(data)){
					 		...
					 		ian.alert('提示', data.msg || "请求出现异常,请联系管理员",'error');
					 	}
			 		}catch(e){
			 			ian.alert('提示',"请求出现异常,请联系管理员.",'error');
			 		}
			 	},
			 	complete:function(){
	...
	submitForm:function(form,callback,dataType){
			var option =
			{
			 ...
			 ian.ajaxSubmit(form,option);
	},
	saveForm:function(form,callback){
		
		if(form.form('validate')){
			ian.progress('请稍候','正在保存...');
			//ajax提交form
	...
	getById:function(url,option,callback){
		ian.progress();
		ian.ajaxJson(url,option,function(data){
			ian.closeProgress();
			if(data.success){
				if(callback){
			       	callback(data);
