var YDataGrid = function(config,Grid){
		config = config || {};
		...
		var Handler = {
			...
			add: function(callback){
				Win.edit.attr('style','display:block');
			...
			edit:  function(callback){
				...
				if (Utils.checkSelectOne(record)){
					...
					ian.getById(Action.getId,data,function(result){
						ian.closeProgress();
				...
				if (Utils.checkSelect(records)){
					$.messager.confirm('确认','确认删除记录?',function(r){  
					    if (r){
					    	...
					    	var data = arr.join("&");
					   		ian.deleteForm(Action.remove,data,function(result){
								ian.closeProgress();
								Events.refresh();
								//回调函数
								if(jQuery.isFunction(callback)){
			...
			save: function(callback){
				if(Form.edit.form('validate')){
					...
					ian.saveForm(Form.edit,function(data){
						ian.closeProgress();
		...
		var bar_check = { id:'btncheck',
						text:'审核',
						iconCls:'icon-edit',
						btnType:'edit',
						handler:Events.check
					   };
		...
		var initTbar = function(){
			...
			//查询页面授权的btnType
			ian.ajaxJson(_url,data,function(data){
				if(data.success){
					if(data.allType){
